...
namespace JumpPoint.Platform.Services
{
    public static partial class SettingLinkService
    {
        ...
        static async Task<ReadOnlyCollection<SettingLink>> PlatformGetSettingLinks()
        {
            return await Task.Run(() =>
            {
                var settings = new List<SettingLink>();
                foreach (var item in PlatformGetSettingLinkTemplates())
                {
                    var setting = PlatformGetSettingLink(item);
                    if (!(setting is null))
                    {
                        settings.Add(setting);
                    }
                }
                return new ReadOnlyCollection<SettingLink>(settings);
            });
        }
        ...
        static async Task PlatformOpen(SettingLinkTemplate template)
        {
            var path = PlatformGetPath(template);
            await Launcher.LaunchUriAsync(new Uri(path));
        }

        static string PlatformGetPath(SettingLinkTemplate template)
        {
            return settingLinks.Value.TryGetValue(template, out var info) ? info.Path : string.Empty;
        }
    
        static SettingLinkGroup PlatformGetGroup(SettingLinkTemplate template)
        {
            return settingLinks.Value.TryGetValue(template, out var info) ? info.Group : SettingLinkGroup.None;
        }
